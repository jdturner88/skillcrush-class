def numerology(birthdate)
	...
	final_number = number[0].to_i + number[1].to_i

	if (final_number > 9)
		then final_number = final_number[0].to_i + final_number[1].to_i
	end

	return final_number
end

def message(final_number)
	case final_number
		when 1
...
puts "Please enter your birthdate in MMDDYYY format"

birthdate = gets

final_number = numerology(birthdate)

message = message(final_number)
